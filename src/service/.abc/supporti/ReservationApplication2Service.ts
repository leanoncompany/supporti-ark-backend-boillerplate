...
export default class ReservationApplication2Service extends ProductApplicationService {
	...
	public async findAvailableTimeByProduct(customizedOption: {
		[key: string]: any;
	}) {
	...
	private async getMonthlyAvailableTimeList(reservationSystem: any) {
		...
		const startMonth = moment(reservationSystem.START_DATE).format('MM');
		const endMonth = moment(reservationSystem.END_DATE).format('MM');
		// 요청 월의 첫 날부터 마지막 날까지 가능한 날짜 셋팅
		let firstDate = moment(reservationSystem.START_DATE);
		let lastDate = moment(reservationSystem.END_DATE);
	...
	private async exceptReservedTime(
		...
		Promise.all(
			reservedApplicationList.map((reservedApplication: any) => {
				const list = JSON.parse(reservedApplication.RESERVATION_DATE);
	...
	private async applyTimeCheck(customizedOption: { [key: string]: any }) {
		const availableDateAndTimeList = await this.findAvailableTimeByProduct({
			[this.reservationSystemKey]:
				customizedOption.CREATE_OPTION_KEY_LIST[
					this.reservationSystemKey
				],
